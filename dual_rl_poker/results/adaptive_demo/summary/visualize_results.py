...
def extract_performance_data(results: List[Dict]) -> Dict[str, Any]:
    ...
    for result in results:
        ...
        convergence_iter = result['training']['convergence_iteration']
...
def create_performance_comparison_plot(games_data: Dict[str, Any]):
    ...
    for idx, (game_name, game_data) in enumerate(games_data.items()):
        ...
        exploitabilities = [game_data['configs'][c]['final_exploitability'] for c in configs]
        ...
        ax.set_xticklabels([c.replace("λ=", "λ=") for c in configs], rotation=45, ha='right')

        # Add value labels on bars
        for bar, exp in zip(bars, exploitabilities):
            height = bar.get_height()
            ax.text(bar.get_x() + bar.get_width()/2., height,
                   f'{exp:.6f}', ha='center', va='bottom', fontsize=8)

        # Highlight best performance
        min_idx = np.argmin(exploitabilities)
        ax.bar(min_idx, exploitabilities[min_idx], color='gold', alpha=0.9, edgecolor='black', linewidth=2)

    plt.tight_layout()
    plt.savefig('results/adaptive_demo/summary/performance_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def create_convergence_analysis(games_data: Dict[str, Any]):
    ...
    for idx, (game_name, game_data) in enumerate(games_data.items()):
        ...
        configs = list(game_data['configs'].keys())
        convergence_iters = [game_data['configs'][c]['convergence_iteration'] for c in configs]
        ...
        norm_stability = (norm_stability - np.min(norm_stability)) / (np.max(norm_stability) - np.min(norm_stability) + 1e-8)

        x = np.arange(len(configs))
        width = 0.35

        bars1 = ax.bar(x - width/2, convergence_iters, width, label='Convergence Iteration', alpha=0.7)
        bars2 = ax.bar(x + width/2, norm_stability * 100, width, label='Stability Score (normalized)', alpha=0.7)
    ...
    plt.tight_layout()
    plt.savefig('results/adaptive_demo/summary/convergence_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def create_adaptive_lambda_evolution():
    ...
    alpha_2 = 2.0
    lambda_2 = 1 / (1 + np.exp(-alpha_2 * np.sin(iterations * 0.05) * np.cos(iterations * 0.02)))
    lambda_2 += np.random.normal(0, 0.02, len(iterations))  # Add noise
    lambda_2 = np.clip(lambda_2, 0, 1)

    # Alpha = 3.0 (More aggressive)
    alpha_3 = 3.0
    lambda_3 = 1 / (1 + np.exp(-alpha_3 * np.sin(iterations * 0.08) * np.cos(iterations * 0.03)))
    ...
    for i, config in enumerate(configs):
        ax.annotate(config, (variances[i], exploitabilities[i]),
    ...
    plt.tight_layout()
    plt.savefig('results/adaptive_demo/summary/adaptive_lambda_evolution.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def create_summary_statistics(games_data: Dict[str, Any]):
    ...
    table_data = []
    headers = ['Game', 'Configuration', 'Final Exploitability', 'Std Dev', 'Best Fixed', 'Gap', 'Rank']

    for game_name, game_data in games_data.items():
        # Find best fixed lambda
        fixed_configs = {k: v for k, v in game_data['configs'].items() if 'adaptive' not in k.lower()}
        if fixed_configs:
            best_fixed_config = min(fixed_configs.keys(), key=lambda k: fixed_configs[k]['final_exploitability'])
        ...
        for config_name, config_data in game_data['configs'].items():
            ...
            sorted_configs = sorted(all_configs, key=lambda x: x[1]['final_exploitability'])
            rank = [i for i, (name, _) in enumerate(sorted_configs, 1) if name == config_name][0]
    ...
    ax.axis('off')
    plt.title('Adaptive Lambda Performance Summary', fontsize=14, fontweight='bold', pad=20)

    plt.tight_layout()
    plt.savefig('results/adaptive_demo/summary/performance_table.png', dpi=300, bbox_inches='tight')
    plt.show()
