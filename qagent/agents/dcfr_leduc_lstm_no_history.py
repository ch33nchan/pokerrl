"""DCFR Leduc ablation without history inputs, with diagnostics instrumentation."""
...
class CardOnlyNet(nn.Module):
    """Strategy head that ignores betting history."""

    def __init__(self, num_cards: int, embedding_dim: int = 32, fc_hidden_size: int = 128) -> None:
        super().__init__()
        self.card_embedding = nn.Embedding(num_cards, embedding_dim)
        self.fc = nn.Sequential(
            nn.Linear(embedding_dim, fc_hidden_size),
            nn.ReLU(),
            nn.Linear(fc_hidden_size, NUM_ACTIONS),
        )

    def forward(self, private_card: torch.Tensor, public_card: torch.Tensor) -> torch.Tensor:
        private_emb = self.card_embedding(private_card)
        public_emb = self.card_embedding(public_card)
        combined = private_emb + public_emb
        return self.fc(combined)


class DCFRAblationNoHistoryTrainer:
    """DCFR trainer variant that receives no betting-history features."""
    ...
    def __init__(
        ...
        log_prefix: str = "leduc_lstm_no_history",
        log_to_csv: bool = True,
        log_to_jsonl: bool = False,
    ) -> None:
        self.game = game if game is not None else LeducHoldem()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.no_card_index = self.game.num_ranks
        num_cards = self.game.num_ranks + 1  # include slot for 'no card'

        self.regret_net = CardOnlyNet(num_cards, embedding_dim, fc_hidden_size).to(self.device)
        self.strategy_net = CardOnlyNet(num_cards, embedding_dim, fc_hidden_size).to(self.device)
        ...
        self.regret_memory: List[Tuple[Tuple[torch.Tensor, torch.Tensor], np.ndarray]] = []
        self.strategy_memory: List[Tuple[Tuple[torch.Tensor, torch.Tensor], np.ndarray]] = []
    ...
    @staticmethod
    def _clone_inputs(inputs: Tuple[torch.Tensor, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        private_card, public_card = inputs
        return private_card.detach().clone(), public_card.detach().clone()

    def _encode_info_set(self, state: Dict[str, object]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        private_card_val = int(private_cards[current_player]) % self.game.num_ranks

        board_card = state.get("board_card")
        if board_card is None:
            public_card_val = self.no_card_index
        else:
            public_card_val = int(board_card) % self.game.num_ranks

        private_card_tensor = torch.tensor([private_card_val], dtype=torch.long, device=self.device)
        public_card_tensor = torch.tensor([public_card_val], dtype=torch.long, device=self.device)
        return private_card_tensor, public_card_tensor
    ...
    def get_strategy(
        ...
        private_card, public_card = inputs
        logits = self.regret_net(private_card, public_card)
    ...
    def get_average_strategy(
        ...
        private_card, public_card = inputs
        logits = self.strategy_net(private_card, public_card).squeeze(0)
    ...
    def _traverse_tree_iterative(self, player_to_train: int) -> None:
        ...
        sampled_info: Dict[str, Tuple[Tuple[torch.Tensor, torch.Tensor], np.ndarray] | int | str] = {}
    ...
    def _update_regret_net(self, batch_size: int, iteration: int) -> Optional[Dict[str, float]]:
        ...
        predicted_regrets = self.regret_net(private_cards, public_cards)
    ...
    def _update_strategy_net(self, batch_size: int, iteration: int) -> Optional[Dict[str, float]]:
        ...
        predicted_logits = self.strategy_net(private_cards, public_cards)
...
__all__ = ["DCFRAblationNoHistoryTrainer"]
